...
def particle_swarm_optimization(best_solution, num_particles=50, max_iterations=1500, w=0.5, c1=1.5, c2=1.5, sound_times_with_error=None):
...
best_solution, fitness_history = genetic_algorithm_with_pso_tracking(sound_times_with_error=new_sound_times)

# Visualize RMSE over generations and iterations
plt.figure(figsize=(10, 6))
plt.plot(fitness_history, label="Fitness(RMSE)", color="blue")
plt.xlabel("迭代次数")
plt.ylabel("适应度(RMSE)")
plt.grid(True)
plt.legend()
...
print("Best explosion times:", explosion_times)plt.rcParams['font.sans-serif'] = ['SimHei']  # 设置全局字体为 SimHei，适用于 xlabel, ylabel, title 等
plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
